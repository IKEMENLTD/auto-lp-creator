...
${extra}
※上記は抽出済みデータ。他社の情報が混入している可能性あり。話者ラベルで必ず検証せよ。`;
}
...
【情報帰属ルール（厳守）】
- 商談には複数の企業/人が参加している。対象企業以外の能力・サービス・実績を対象企業のものとして記載してはならない
- 話者ラベルで「誰が言った情報か」を必ず確認。対象企業の話者ラベル以外の発言に含まれる実績・サービスは他社のもの
- 「協業」「連携」で紹介された他社の能力は、対象企業のサービスではない
- 例: A社のページを作る時、B社が「うちは開発が安い」と言った情報はA社のサービスに含めない
...
【情報帰属チェック（厳守）】
- services/strengths/statsに「対象企業以外の会社の能力」が混入していないか検証せよ
- 商談相手が自社サービスとして語った内容（開発費、サブスク、ソースコード等）を対象企業のサービスとして記載していたら削除・差し替え
- 「協業先の能力」と「対象企業自身の能力」を明確に区別
