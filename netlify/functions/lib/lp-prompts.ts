...
10. cases: 最低2件必須。トランスクリプトに言及あればそのまま使用。不足分は業種の典型的な成功パターンで補完（「某○○企業」等の匿名表記）。個人名（担当者名・顧客名）は絶対に含めない。「担当者」「クライアント」等に置き換えること
11. stats: 4つ異なるカテゴリ。numberは必ず数値を含めること（例: "92%", "300件+", "50社以上", "24時間"）。数字のない抽象的な表現は禁止
...
17. cta_text: 8字以内。矢印(→>＞)は含めずテキストのみ
